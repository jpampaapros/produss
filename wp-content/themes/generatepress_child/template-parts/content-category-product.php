...
$termDescription = $queried_object->description;
$tab = $_GET["tab"];
...
$porDefecto = $terms[0];
if(empty($tab)) $tab = $porDefecto->slug;
if(empty($tab)) $tab = $termSlug;
$postSlug = $porDefecto->slug;
?>
<?php if(true):?>
    <?php
    ...
    ?>
    <section class="pro-listado-con-tabs-productos">
        <div class="pro-text-center pro-color-white pro-bg-blue pro-banner-title pro-relative pro-o-hidden pro-mov-px-spacing">
            <div class="pro-section-content pro-relative">
                ...
                <?php if($termName):?>
                    <h2 class="pro-section__title pro-title pro-title-line-center pro-only-desk"><?= $termName;?></h2>
                    <div class="pro-section__title pro-title pro-mov-text-left pro-title-line pro-only-mov"><?= $termName;?></div>
                <?php endif;?>
                <?php if($termDescription):?>
                    <div class="pro-section__text pro-text pro-mov-text-left"><?= $termDescription;?></div>
                <?php endif;?>
                <?php if($terms):?>
                    <div class="pro-group-btns">
                        <?php foreach ($terms as $term):?>
                            <a class="pro-btn pro-minw-initial <?= ($term->slug === $tab ? 'pro-bg-orange pro-color-blue' : 'pro-btn-white-outline');?>" href="?tab=<?= $term->slug;?>"><?= $term->name;?></a>
                        <?php endforeach;?>
                    </div>
                <?php endif;?>
            </div>
